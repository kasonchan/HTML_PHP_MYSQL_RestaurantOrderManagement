...
{
	$totalAmount = getTotalAmount($table_number) + getTipsAmount($table_number);
...
function getOccupied($table_number)
{
	$getOccupied = "SELECT occupied FROM table_status WHERE table_number='$table_number'";
	$getOccupiedResult = mysql_query($getOccupied);

	if($getOccupiedResult)
	{
		while($row=mysql_fetch_row($getOccupiedResult))
		{
      			foreach ($row as $value)
			{
				return $value;
...
function getMyTable($table_number)
{
	$getMyTable = "SELECT assign_to FROM table_status WHERE table_number='$table_number'";
	$getMyTableResult = mysql_query($getMyTable);

	if($getMyTable)
	{
		while($row=mysql_fetch_row($getMyTableResult))
		{
      			foreach ($row as $value)
			{
				return $value;

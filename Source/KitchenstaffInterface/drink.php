...
<div style="padding-left: 40%"><h>Drink</h><br></div>
<div>
...
<?php
	...
	$getIngredientName = "SELECT distinct ingredient_name FROM ingredient WHERE ingredient_type = 'Dressing'";
	...
	{ 
		...
		if($getIngredientNameResult)
		{
			while($row=mysql_fetch_row($getIngredientNameResult))
			{
      			foreach ($row as $value)
				{ 
					if(!(is_numeric($_POST[$value]) AND ($_POST[$value] >= 0) AND ($_POST[$value])))
					{
						$error = true;
						...
						foreach($row2 as $amount)
						{
							...
							$symbols = array(" ");
  							$value = str_replace($symbols, "_", $value);
								...
								echo "<input type=\"text\" name=\"$value\" value=\"$_POST[$value]\" text-align = \"right\" class = \"Box2\" maxlength=\"3\"/>" . "</h2></div>";
						}	
					}
				}
